...
void main() {
  ...
  test('add list remove round-trip per feature', () async {
    ...
    await SavedViewsStorage.add(SavedViewsStorage.featureReport, 'Q1',
        <String, dynamic>{'type': 'month'});
    await SavedViewsStorage.add(SavedViewsStorage.featureInsights, 'Week',
        <String, dynamic>{'period': 'thisWeek'});

    final List<SavedViewRecord> r =
        SavedViewsStorage.listFor(SavedViewsStorage.featureReport);
    expect(r.length, 1);
    expect(r.first.name, 'Q1');
    expect(r.first.payload['type'], 'month');

    expect(
        SavedViewsStorage.listFor(SavedViewsStorage.featureInsights).length, 1);
