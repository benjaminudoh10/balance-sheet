...
class TransactionController extends GetxController {
  ...
  updateTransaction(
      Transaction transaction, Transaction previousTransaction) async {
    ...
    await notifyBudgetCompletionsAfterTransactionSave(update,
  ...
  deleteTransaction(Transaction transaction) async {
    ...
    if (res == 1) {
      // remove txn from UI
      transactions.value =
          transactions.where((txn) => transaction.id != txn.id).toList();
  ...
  updateControllerDataAfterDeletion(Transaction transaction) {
    ...
    try {
      ...
      reportController.transactions.value = reportController.transactions
          .where((txn) => transaction.id != txn.id)
          .toList();
      if (transaction.type == TransactionType.expenditure) {
        reportController.expense.value -= transaction.amount;
      } else {
        reportController.income.value -= transaction.amount;
      }
    } catch (error) {
      debugPrint('$error');
    }
  ...
  updateControllerDataAfterUpdate(
      Transaction transaction, Transaction previousTransaction) {
    resetFieldValues();

    try {
      final ReportController reportController = Get.find();

      final int index = reportController.transactions.indexWhere(
        (txn) => txn.id == transaction.id,
      );
      if (index != -1) {
        reportController.transactions[index] = transaction;
        if (transaction.type == TransactionType.expenditure) {
          reportController.expense.value +=
              (transaction.amount - previousTransaction.amount);
        } else {
          reportController.income.value +=
              (transaction.amount - previousTransaction.amount);
        }
      }
    } catch (error) {
